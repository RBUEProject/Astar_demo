...
void AAStar_AIController::BeginPlay()
{
	Super::BeginPlay();
...
ATestPawn* AAStar_AIController::GetControlledPawn() const
{
	return Cast<ATestPawn>(GetPawn());
}

void AAStar_AIController::AStar()
{
...
FVector2D AAStar_AIController::WorldCordinatesToMapIndex(FVector2D WorldLocation)
{
...
FVector AAStar_AIController::MapIndexToWorldLocation(FVector2D MapIndex)
{
...
bool AAStar_AIController::CheckMap(FVector2D V)
{
	return V.X >= 0 && V.X < MAP_WIDTH && V.Y >= 0 && V.Y < MAP_HEIGHT;
}

float AAStar_AIController::Manhattan(FVector2D _Dest)
{
...
void AAStar_AIController::pathFinder(FVector2D position)
{
	Path.Empty();
	Path.Push(Dest);
...
bool AAStar_AIController::CheckWalls(FVector2D position)
{
	...
	if (!(!HitResult1.GetActor() && !HitResult2.GetActor() && !HitResult3.GetActor() && !HitResult4.GetActor()))
	{
		map[position.Y][position.X] = -2;
	}
	return !HitResult1.GetActor() && !HitResult2.GetActor() && !HitResult3.GetActor() && !HitResult4.GetActor();
}

bool AAStar_AIController::CheckEdges(FVector2D position, FVector2D movedir)
{
...
void AAStar_AIController::Movement(float DeltaTime)
{
	if (Path.Num() > 1)
	{
		auto MyLocation = GetControlledPawn()->GetActorLocation();
		...
		Dir.Normalize();
		GetControlledPawn()->Movement(Dir, DeltaTime);
	}
}

//76*76 array map from -1900 to 1900
void AAStar_AIController::Tick(float DeltaTime)
{
